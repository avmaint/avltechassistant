...
-   **Connectivity Diagram Rendering:**
    ...
    -   **Crossing Minimisation (Barycenter Heuristic):** For each node's out-ports (right/east side), the port order is further refined using the barycenter heuristic: each out-port is ranked by the average vertical rank of the in-ports it connects to on the neighbouring node. This is the standard algorithm for minimising edge crossings in layered bipartite graphs. Natural sort is used as the tiebreaker when two out-ports have the same barycenter value. The barycenter is applied in `compute_graph_elements` (Cytoscape/ELK path); the DOT path relies on Graphviz's own crossing minimiser and applies only the natural sort.

...
-   **Output Display:**
    ...
    -   **Asset Details Tab:**
        ...
        -   Below the property tables, sections render in this order: **Network**, **Installed Assets** (Computer/Software only, see below), **Input/Output Partners**, **Knowledge Base Issues**, **Manuals**, **Licenses**.
        ...
        -   **Installed Assets:** Assets of `Category` = `Software` record what they are installed on via their own `Location` field (holding the asset tag of the host).
            -   If the subject asset's `Category` is `Computer`, an **Installed Software** table lists every asset with `Category` = `Software` whose `Location` matches this asset's tag, with columns Asset Tag (clickable, navigates like a partner link), Manufacturer, Model, Description, and Usage.
            -   If the subject asset's `Category` is `Software`, two tables are shown instead: **Installed On** — the single asset (any category) referenced by this asset's own `Location` field; and **Software Installed On This Asset** — every `Category` = `Software` asset whose `Location` matches this asset's tag, using the same columns as Installed Software. This is necessary because software can be installed on other software (e.g. a runtime installed inside a container platform installed on a computer), so a software asset can appear as both the "installed" item in one table and the "host" in another.
            -   For any other `Category`, the Installed Assets section is omitted entirely (no heading, no empty-state message).
            -   Backend: `GET /assets/{asset_tag}/details` includes `installed_software` (list) and `installed_on` (list, 0 or 1 entries) fields alongside the existing `asset`/`input_partners`/`output_partners`/`network`/`licenses` fields.

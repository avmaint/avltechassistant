...
- Both are called unconditionally inside `GET /assets/{asset_tag}/details` and returned as `installed_software` (list) and `installed_on` (list, 0–1 entries) alongside the endpoint's existing fields. The endpoint does not gate on category server-side — it always computes both lists cheaply; the frontend decides what to render based on the subject's `Category`. This mirrors how `input_partners`/`output_partners`/`network` are already always computed regardless of whether a given asset has any.
...
- Gated on `asset.Category` (case-insensitive): `computer` → one "Installed Software" table; `software` → an "Installed On" table (parent) followed by a "Software Installed On This Asset" table (children); any other category → section stays empty, no heading rendered at all (distinct from the empty-but-labeled state used elsewhere, e.g. "No network records found").
